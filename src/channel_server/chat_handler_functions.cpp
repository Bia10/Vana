...
auto chat_handler_functions::initialize() -> void {
	...
	command.notes.push_back(" lNNN: write an int64 (long), where NNN = number from " + vana::util::str::lexical_cast<string>(INT64_MIN) + " to " + vana::util::str::lexical_cast<string>(INT64_MAX) + " inclusive");
	command.notes.push_back(" iNNN: write an uint32 (unsigned int), where NNN = number from 0 to " + vana::util::str::lexical_cast<string>(UINT32_MAX) + " inclusive");
	command.notes.push_back(" sNNN: write an uint16 (unsigned short), where NNN = number from 0 to " + vana::util::str::lexical_cast<string>(UINT16_MAX) + " inclusive");
	command.notes.push_back(" bNNN: write an uint8 (unsigned byte), where NNN = number from 0 to " + vana::util::str::lexical_cast<string>(UINT8_MAX) + " inclusive");
